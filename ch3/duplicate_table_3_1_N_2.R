#
# Duplicates Table 3.1 and 3.2 from the book
#
# Written by:
# -- 
...
res = load_prostate_data(globalScale=FALSE,trainingScale=TRUE,responseScale=FALSE)
XTraining = res[[1]]
XTesting = res[[2]] 
...
# reproduce/verify the above above results using R's "lm" function:
# 
Db = cbind( D, lpsa ) 
Df = data.frame( Db )
...
F = data.frame( Term=names(coefficients(m0)), Coefficients=betaHat, Std_Error=stdBetaHat, Z_Score=z )
